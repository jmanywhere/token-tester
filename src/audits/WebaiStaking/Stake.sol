...
abstract contract IERC20Staking is ReentrancyGuard, Ownable {
    ...
    mapping(uint256 => mapping(address => uint256))
        public totalRewardsPerWalletPerPlan;
    ...
    function unstake(
        ...
        if (tamount > 0) {
            IERC20(stakingToken).transfer(
                msg.sender,
                tamount - _penalty + _earned
            );
            totalRewardsPerWalletPerPlan[_stakingId][
                msg.sender
            ] = totalRewardsPerWalletPerPlan[_stakingId][msg.sender].add(
                _earned
            );
        }
